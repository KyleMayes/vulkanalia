...
fun Element.getElementText(tag: String): String? =
    getElement(tag)?.textContent

/** Gets the first child element in this element with the supplied tag. */
fun Element.getElement(tag: String): Element? =
    getElements(tag).firstOrNull()

/** Gets and maps all of the child elements in this element with the supplied tag. */
fun <T> Element.getElements(tag: String, transform: (Element) -> T?): List<T> =
    getElements(tag).mapNotNull(transform)

/** Gets all of the child elements in this element with the supplied tag. */
fun Element.getElements(tag: String): List<Element> =
    getElementsByTagName(tag).mapNodes { it as Element }
...
fun <T> Node.queryElements(@Language("XPath") expr: String, transform: (Element) -> T?): List<T> =
    queryElements(expr).mapNotNull(transform)

/** Finds the elements in this node that match the supplied XPath expression. */
fun Node.queryElements(@Language("XPath") expr: String): List<Element> {
...
fun <T> NodeList.mapNodes(transform: (Node) -> T): List<T> =
    (0..length).mapNotNull(::item).map(transform)
