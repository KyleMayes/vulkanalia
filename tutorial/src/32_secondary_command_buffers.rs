...
fn create_pipeline(device: &Device, data: &mut AppData) -> Result<()> {
    ...
    let vert = include_bytes!("../shaders/30/vert.spv");
    let frag = include_bytes!("../shaders/30/frag.spv");
...
fn create_command_pools(instance: &Instance, device: &Device, data: &mut AppData) -> Result<()> {
    // Global
...
fn create_command_pool(instance: &Instance, device: &Device, data: &mut AppData) -> Result<vk::CommandPool> {
    let indices = QueueFamilyIndices::get(instance, data, data.physical_device)?;
...
fn end_single_time_commands(device: &Device, data: &AppData, command_buffer: vk::CommandBuffer) -> Result<()> {
    // End
